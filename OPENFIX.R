################################################################################
#
# R code for OPENFIX model
#
# Author: Marco Veronese Passarella
#
# Last change: 12 November 2023 
#
# Note: this R code replicates the OPENFIX model presented and used in W. Godley
...
for (i in 2:nPeriods){
  ...
  for (iterations in 1:300){
    ...
    # UK Exchange rate - eq. 12.89F
    b_ukus_s[,i] = b_ukus_d[,i] * xr_uk[,i]
    
    # Supply of UK bills to CountryS - eq. 12.90F
    b_cb_ukus_s[,i] =  b_us_s[,i] - b_usus_s[,i] - b_cb_usus_d[,i] - b_ukus_s[,i]
